...
class TemplateEngine {
    ...
    processTplPipes(tplExp, data) {
        let pipes = tplExp.pipes.map(p => {
            return ((p.pipeParameters = p.pipeParameters.map(param => {
                if (this.regExpStringPipeParameter.test(param))
                    return param.slice(1, -1);
                else if (data[param] || data[param] === 0)
                    return data[param];
                else
                    return param.split(".").reduce((o, key) => ((o && o[key]) || o[key] === 0 ? o[key] : null), this.data);
            })),
                p);
        });
        return pipes;
    ...
    blockPipeFilter(dataArray, propertyName, propertyValue) {
        if (Array.isArray(dataArray) && propertyName) {
            if (propertyValue) {
                return dataArray.filter(item => typeof item === "object" && item[propertyName] && item[propertyName].length > 0 && item[propertyName] === propertyValue);
            }
